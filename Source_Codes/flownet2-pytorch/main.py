...
if __name__ == '__main__':
        ...
        for batch_idx, (data, target) in enumerate(progress):
            ...
            if args.save_flow or args.render_validation:
                for i in range(args.inference_batch_size):
                    ...
                    flow_utils.writeFlow( join(flow_folder, '%05d.flo'%(batch_idx * args.inference_batch_size + i)),  _pflow)
    ...
    for epoch in progress:
        if args.inference or (args.render_validation and ((epoch - 1) % args.validation_frequency) == 0):
            ...
            # Gets length of dataset (Number of pairs of frames)
            n_frame_pairs = len(inference_loader)
            print('Number of frame pairs: {}'.format(n_frame_pairs))
            ...
            print('Sequence evaluation time: {}s'.format(end - start))

            # Appends n_frame_pairs and time duration to file
            with open('time_benchmark.txt', 'a+') as f:
                f.write("%d %.8f\n" % (n_frame_pairs, end-start))
